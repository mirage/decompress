...
let blit src src_off dst dst_off len =
  ...
  then
    invalid_arg "Bytes.blit (src: %d:%d, dst: %d:%d, len: %d)" src_off
...
let blit2 src src_off dst0 dst_off0 dst1 dst_off1 len =
  ...
  then
    invalid_arg "Bytes.blit2 (src: %d:%d, dst0: %d:%d, dst1: %d:%d, len: %d)"
      src_off (length src) dst_off0 (length dst0) dst_off1 (length dst1) len
  else
    for i = 0 to len - 1 do
