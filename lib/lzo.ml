...
let get_char buf ofs =
  if ofs < 0 || ofs > bigstring_length buf - 1
...
let get_int16 buf ofs =
  if ofs < 0 || ofs > bigstring_length buf - 2
  then raise Out_of_bound ;
...
external unsafe_set_int8  : bigstring -> int -> int -> unit = "%caml_ba_unsafe_set_1"
...
let get_int32 buf ofs =
  if ofs < 0 || ofs > bigstring_length buf - 4
  then raise Out_of_bound ;
  unsafe_get_int32 buf ofs

let get_int64 buf ofs =
  if ofs < 0 || ofs > bigstring_length buf - 8
  then raise Out_of_bound ;
  unsafe_get_int64 buf ofs

external unsafe_set_int32 : bigstring -> int -> int32 -> unit = "%caml_bigstring_set32"
...
let blit src src_off dst dst_off len =
  if len < 0 || src_off < 0 || src_off > bigstring_length src - len
             || dst_off < 0 || dst_off > bigstring_length dst - len
  then raise Out_of_bound ;
  unsafe_blit src src_off dst dst_off len

external bytes_unsafe_set_int32 : bytes -> int -> int32 -> unit = "%caml_bytes_set32u"
...
let bigstring_to_string buf ofs len =
  if ofs < 0 || len < 0 || ofs > bigstring_length buf - len
  then raise Out_of_bound ;
  unsafe_bigstring_to_string buf ofs len

let unsafe_get_int16_le =
  if Sys.big_endian
  then fun buf off -> swap16 (unsafe_get_int16 buf off)
  else fun buf off -> unsafe_get_int16 buf off

let unsafe_get_int16_be =
  if Sys.big_endian
  then fun buf off -> unsafe_get_int16 buf off
...
let copy_to_buffer buf v ~off ~len =
  ...
  if Buffer.length buf >= off
  then go off len
  else Error `Invalid_dictionary
...
let count t =
  ...
  while not (!idx > max - 4) && get_int32 t.i !idx = 0l
  do idx := !idx + 4 ; res := !res + 4 done ;
  while !idx + 1 <= max && get_int8 t.i !idx = 0
  do incr idx ; incr res done ;
...
type ('a, 'b) k =
  | Ok of 'a | Error of 'b | Kontinuation of (unit -> ('a, 'b) t)

let ( >>= ) : ('a, 'err) result -> ('a -> ('b, 'err) result) -> ('b, 'err) result = fun  x f -> match x with
  | Ok x -> f x
  | Error err -> Error err
...
let run
  ...
  = fun ~transmit ~copy fiber t ->
    let rec go
      ...
      | Junk Byte ->
        if t.i_pos < bigstring_length t.i
        then ( t.i_pos <- t.i_pos + 1 ; Ok () ) else raise Out_of_bound
      | Peek (Short endian) ->
        Ok (get_int16 t.i t.i_pos endian)
      | Junk (Short _) ->
        if t.i_pos + 1 < bigstring_length t.i
        then ( t.i_pos <- t.i_pos + 2 ; Ok () ) else raise Out_of_bound
      | Bind (x, f) ->
        ( match go t x with
        | Ok v -> go t (f v)
        | Error _ as err -> err
      ...
      | Fix fix ->
        ...
        let rec m = lazy (fix r)
        and r = Lazy m in
        go t r
      | State -> Ok t.state
      | Count ->
        ( match count t with Ok v -> Ok v | Error err -> Error err )
      | Transmit (len, state) ->
        t.state <- state ;
      ...
      | Copy ({ off; len; }, state) ->
        t.state <- state ;
        let fiber =
          copy t ~off ~len:(len + 2) >>= fun () ->
          copy_done ~transmit t in
        match fiber with
        | Ok v -> Ok v
        | Error err -> Error err in
    let rec unroll t fiber : _ result = match go t fiber with
      | Ok v -> Ok v
      | Error err -> Error err
...
module DSL = struct
  ...
  let read v = peek v >>= fun r -> junk v >>= fun () -> return r
end
...
let fiber : (unit, [> error ]) t =
  ...
  | '\001' .. '\015', 0 ->
    transmit (Char.code chr + 3) state >>= fun () -> m
  | '\000', 0 ->
    count >>= fun count ->
  ...
  | '\016' .. '\031', _ ->
    ...
    let with_length len =
      ...
      if off = 16384 then end_of_lzo else copy ~off ~len state >>= fun () -> m in
    if length = 0
    then count >>= fun count -> with_length (7 + count)
  ...
  | '\032' .. '\063', _ ->
    ...
    if length = 0
    then count >>= fun count -> with_length (31 + count)
    else with_length length
  | '\064' .. '\255', _ ->
    let state = State.of_int (Char.code chr) in
    let len, d =
      (Char.code chr lsr 5) - 1, (* t = (t >> 5) - 1 *)
      ((Char.code chr lsr 2) land 7) (* m_pos = (t >> 2) & 7 *)in
    read byte >>= fun h ->
    let off = (Char.code h lsl 3) + d + 1 in
    copy ~off ~len state >>= fun () -> m
  | _ -> assert false (* TODO: replace it by something else to ensure exhaustive pattern-matching. *)
...
let fiber : (unit, [> error ]) t =
  ...
  | '\000' .. '\017' -> fiber
  | '\018' -> junk byte >>= fun () -> transmit ~len:1 State._0 >>= fun () -> fiber
  | '\019' -> junk byte >>= fun () -> transmit ~len:2 State._0 >>= fun () -> fiber
  | '\020' -> junk byte >>= fun () -> transmit ~len:3 State._0 >>= fun () -> fiber
  | '\021' -> junk byte >>= fun () -> transmit ~len:4 State._0 >>= fun () -> fiber
  | '\022' .. '\255' as chr ->
...
let get_int32_le =
  if Sys.big_endian
  then fun buf off -> swap32 (get_int32 buf off)
  else fun buf off -> get_int32 buf off

let record_match ~off ~len out_data _anchor out_pos =
  let out_pos = ref out_pos in

  if len <= _m2_max_len && off <= _m2_max_offset
  then ( let off = off - 1 in
         out_data.%[!out_pos] <- ((len - 1) lsl 5) lor ((off land 7) lsl 2) ; incr out_pos ;
         out_data.%[!out_pos] <- off asr 3 ; incr out_pos )
  else if off <= _m3_max_offset
  then ( let off = off - 1 in
         if len <= _m3_max_len
         then ( out_data.%[!out_pos] <- _m3_marker lor (len - 2) ; incr out_pos )
         else
           ( let len = ref (len - _m3_max_len) in
             out_data.%[!out_pos] <- _m3_marker lor 0 ; incr out_pos
           ; while !len > 255
             do len := !len - 255 ; out_data.%[!out_pos] <- 0 ; incr out_pos done ;
             out_data.%[!out_pos] <- !len ; incr out_pos ) ;
         out_data.%[!out_pos] <- off lsl 2 ; incr out_pos ;
         out_data.%[!out_pos] <- off asr 6 ; incr out_pos )
  else
    ( let off = off - 0x4000 in
      if len <= _m4_max_len
      then ( out_data.%[!out_pos] <- _m4_marker lor ((off asr 11) land 8) lor (len - 2) ; incr out_pos )
      else ( let len = ref (len - _m4_max_len) in
             out_data.%[!out_pos] <- _m4_marker lor ((off asr 11) land 8) ; incr out_pos ;
             while !len > 255
             do len := !len - 255 ; out_data.%[!out_pos] <- 0 ; incr out_pos done ;
             out_data.%[!out_pos] <- !len ; incr out_pos ) ;
      out_data.%[!out_pos] <- off lsl 2 ; incr out_pos ;
...
let record_literals ~off ~len in_data out_data _anchor out_pos =
  ...
  if len > 0
  then ( if len <= 3
         then ( out_data.%[!out_pos - 2] <- out_data.%[!out_pos - 2] lor len
              ; unsafe_blit in_data off out_data !out_pos 4
              ; out_pos := !out_pos + len )
         else if len <= 16
         then ( out_data.%[!out_pos] <- (len - 3)
              ; incr out_pos
           ...
           ( if len <= 18
             then ( out_data.%[!out_pos] <- len - 3 ; incr out_pos )
             else ( let len' = ref (len - 18) in
                    out_data.%[!out_pos] <- 0 ; incr out_pos
                  ; while !len' > 255
                    do len' := !len' - 255 ; out_data.%[!out_pos] <- 0 ; incr out_pos done
                  ; out_data.%[!out_pos] <- !len' ; incr out_pos )
...
let record_trailer ~off ~len in_data out_data out_pos =
  ...
  if len > 0
  then ( if !out_pos = 0 && len < 238
         then ( out_data.%[!out_pos] <- 17 + len ; incr out_pos )
         else if len <= 3
         then ( out_data.%[!out_pos - 2] <- out_data.%[!out_pos - 2] lor len )
         else if len <= 18
         then ( out_data.%[!out_pos] <- len - 3 ; incr out_pos )
           ...
           ( let len' = ref (len - 18) in
             out_data.%[!out_pos] <- 0 ; incr out_pos ;
             while !len' > 255 do len' := !len' - 255 ; out_data.%[!out_pos] <- 0 ; incr out_pos done ;
             out_data.%[!out_pos] <- !len' ; incr out_pos ) ;
         unsafe_blit in_data off out_data !out_pos len ) ;
  out_pos := !out_pos + len ;
  out_data.%[!out_pos] <- _m4_marker lor 1 ; incr out_pos ;
  out_data.%[!out_pos] <- 0 ; incr out_pos ;
  out_data.%[!out_pos] <- 0 ; incr out_pos ;
  !out_pos
...
let compress in_data in_pos in_len out_data out_pos out_len t wrkmem =
  ...
  let rec literal idx0 idx1 op t =
    (* literal: *)
    let idx0 = idx0 + (1 + ((idx0 - idx1) asr 5)) in (* TODO: check [lsr]. *)
    next idx0 idx1 op t
  and next idx0 idx1 op t =
    (* next: *)
    if idx0 - in_pos >= idx_end
    then (* break *)
      ( let idx1 = idx1 - t and t = 0 in
        (in_len - (idx1 - in_pos - t), op) )
    else
      let v =
        get_int32_le in_data idx0 in
      let index = Int32.(logand (shift_right (mul 0x1824429dl v) (32 - 14)) (sub (shift_left 1l 14) 1l)) in
      let index = Int32.to_int index in
      let reference = Bigarray.Array1.get wrkmem index + in_pos in
      Bigarray.Array1.set wrkmem index (idx0 - in_pos) ;

      if v <> get_int32_le in_data reference
      then literal idx0 idx1 op t
      else
        ( let idx1 = idx1 - t in let t = 0 in
          let unrecorded = idx0 - idx1 in
          let op, idx1 = record_literals ~off:idx1 ~len:unrecorded in_data out_data out_pos op in

          let len = ref 4 in
          while idx0 + !len - in_pos < idx_end
                && get_int64 in_data (idx0 + !len) = get_int64 in_data (reference + !len)
          do len := !len + 8 done
          (* XXX(dinosaure): it seems that [minilzo] does not call [ctz] at the end of [progl].
             May be we do an unsafe access! TODO! *)
        ; if idx0 + !len - in_pos < in_len
          then len := !len + (ctz (Int64.logxor (get_int64 in_data (idx0 + !len)) (get_int64 in_data (reference + !len))) / 8)
        ; let op = record_match ~off:(idx0 - reference) ~len:!len out_data out_pos op in
          next (idx0 + !len) (idx0 + !len) op t ) in
  let idx0 = in_pos + (if t < 4 then 4 - t else 0) in
  literal idx0 in_pos out_pos t

type wrkmem = (int, Bigarray.int16_unsigned_elt, Bigarray.c_layout) Bigarray.Array1.t

let make_wrkmem () = Bigarray.Array1.create Bigarray.int16_unsigned Bigarray.c_layout (1 lsl 14)

module Wrkmem = struct
  let memset t v = Bigarray.Array1.fill t v
end

let compress in_data in_len out_data out_len wrkmem =
  let rec go idx len out_pos t =
    if len <= 20
    then trailer idx len out_pos t
    else (* len > 20 *)
      ( let ll = min len 49152 in
        if (t + ll) lsr 5 <= 0 then trailer idx len out_pos t
        else
          ( Wrkmem.memset wrkmem 0
          ; let t, out_pos = compress in_data idx ll out_data out_pos out_len t wrkmem in
...
let compress in_data out_data wrkmem =
  Wrkmem.memset wrkmem 0 ;
  compress
    in_data (bigstring_length in_data)
    out_data (bigstring_length out_data)
    wrkmem
