...
let compress ?(level = 4) data =
  ...
  (* We need to allocate an input buffer, the size of this buffer is important.
     ...
     If it is big, the algorithm can find a far pattern and keep this pattern as
     long as it tries to compress. The optimal size seems to be [1 << 15] bytes
     (a bigger buffer is not necessary because the distance can be upper than [1
     << 15]). *)
  ...
  (* This is the same interface as [caml-zip]. A refiller and a flusher. The
     ...
     how many bytes are free in the output buffer (and the common is that all is
     free).

     One argument (optionnal) is missing, it's the [meth]. This argument is used
     to limit the memory used by the state internally. In fact, Decompress (and
     `zlib`) need to keep all of your input to calculate at the end the
     ...
     (with SYNC, PARTIAL or FULL - see the documentation about that) at each [n]
     bytes, like: ~meth:(PARTIAL, 4096) flushes the internal memory when we
...
let uncompress data =
  let input_buffer = Bytes.create 0xFFFF in
  (* We need to allocate an input buffer. it's depends your capabilities of your
     reading. *)
  let output_buffer = Bytes.create 0xFFFF in
  (* Same as [compress]. *)
